                ...
                for my $opt_type (0, 1) {
                    ...
                    for my $thresh (5, 10, 15, 20, 30, 50, 70, 100, 150, 200, 250) {
                        ...
                        for my $rank (1, 2, 3, 5, 7, 10, 20, 30, 50) {
                            ...
                            my $this_file_name = "$input_dir/$func.$file_name.exp$expnum..$num_frames.$width.$height.$group_size.$rank.$thresh.$opt_swap_mat.$opt_type.txt";
                            ...
                            while(<FH>) {
                                ...
                                $data{TRACE}{"$file_name.exp$expnum."}{OPT_SWAP_MAT}{$opt_swap_mat}{GROUP_SIZE}{$group_size}{RANK}{$rank}{OPT_TYPE}{$opt_type}{THRESH}{$thresh}{TP} = $tp;
                                $data{TRACE}{"$file_name.exp$expnum."}{OPT_SWAP_MAT}{$opt_swap_mat}{GROUP_SIZE}{$group_size}{RANK}{$rank}{OPT_TYPE}{$opt_type}{THRESH}{$thresh}{TN} = $tn;
                                $data{TRACE}{"$file_name.exp$expnum."}{OPT_SWAP_MAT}{$opt_swap_mat}{GROUP_SIZE}{$group_size}{RANK}{$rank}{OPT_TYPE}{$opt_type}{THRESH}{$thresh}{FP} = $fp;
                                $data{TRACE}{"$file_name.exp$expnum."}{OPT_SWAP_MAT}{$opt_swap_mat}{GROUP_SIZE}{$group_size}{RANK}{$rank}{OPT_TYPE}{$opt_type}{THRESH}{$thresh}{FN} = $fn;

                                $data{TRACE}{"$file_name.exp$expnum."}{OPT_SWAP_MAT}{$opt_swap_mat}{GROUP_SIZE}{$group_size}{RANK}{$rank}{OPT_TYPE}{$opt_type}{THRESH}{$thresh}{PRECISION} = $precision;
                                $data{TRACE}{"$file_name.exp$expnum."}{OPT_SWAP_MAT}{$opt_swap_mat}{GROUP_SIZE}{$group_size}{RANK}{$rank}{OPT_TYPE}{$opt_type}{THRESH}{$thresh}{RECALL} = $recall;
                                $data{TRACE}{"$file_name.exp$expnum."}{OPT_SWAP_MAT}{$opt_swap_mat}{GROUP_SIZE}{$group_size}{RANK}{$rank}{OPT_TYPE}{$opt_type}{THRESH}{$thresh}{F1SCORE} = $f1score;

                                my $buf = "$file_name, $expnum, $num_frames, $width, $height, $opt_swap_mat, $group_size, $rank, $thresh, $opt_type, $tp, $tn, $fp, $fn, $precision, $recall, $f1score\n";
                                print $buf;
                                print FH_OUT $buf;
