...
sub plot_anomaly_size {
    ...
    my $y_max = get_y_max("AnomalySize", $drop_mode, $file_name);
    my $cmd = "sed 's/FILE_NAME/pred.$output_file/g; s/FIG_NAME/pred.$output_file/g; s/X_RANGE_S/0/g; s/X_RANGE_E//g; s/Y_RANGE_E/$y_max/g; s/X_LABEL/Anomaly Size s/g; s/Y_LABEL/NMAE/g; ' $plot_file > tmp.plot.pred.plot";
    `$cmd`;
    ...
    if($drop_mode eq "ind") {
        ...
        # `$cmd`;
        my $cmd = "sed 's/FILE_NAME/dect.$output_file/g; s/FIG_NAME/dect.$output_file/g; s/X_RANGE_S/0.2/g; s/X_RANGE_E//g; s/Y_RANGE_E/1/g; s/X_LABEL/Anomaly Size s/g; s/Y_LABEL/F1-Score/g; ' plot.pred.mother.plot > tmp.plot.dect.plot";
        `$cmd`;
...
sub get_y_max {
    ...
    if($type eq "AnomalySize") {
        if($drop_mode eq "ind") {
            ...
            elsif($file_name eq "tm_telos_rssi.txt")                         { $y_max = 1.1; }
            elsif($file_name eq "tm_multi_loc_rssi.txt")                     { $y_max = 0.202; }
            elsif($file_name eq "tm_ron1.latency.")                          { $y_max = 0.7; }
        }
        else {
            ...
            elsif($file_name eq "tm_telos_rssi.txt")                         { $y_max = 0.35; }
