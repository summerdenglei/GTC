...
sub get_y_max {
    ...
    if($type eq "AnomalySize") {
        if($drop_mode eq "ind") {
            ...
            elsif($file_name eq "tm_totem.")                                 { $y_max = 2.6; }
            ...
            elsif($file_name eq "tm_telos_rssi.txt")                         { $y_max = 1.2; }
            elsif($file_name eq "tm_multi_loc_rssi.txt")                     { $y_max = 0.3; }
            elsif($file_name eq "tm_ron1.latency.")                          { $y_max = 1.7; }
            elsif($file_name eq "tm_ucsb_meshnet.connected.txt")             { $y_max = 2.5; }
            elsif($file_name eq "tm_umich_rss.txt")                          { $y_max = 0.15; }
            elsif($file_name eq "static_trace13.ant1.mag.txt")               { $y_max = 0.8; }
        }
        else {
