                            ...
                            for my $sigma_mag (@sigma_mags) {
                                for my $sigma_noise (@sigma_noises) {
                                    for my $thresh (@threshs) {
                                        ...
                                        plot_prediction($func, $file_name, $num_frames, $width, $height, $group_size, $rank, $period, $opt_swap_mat, $opt_dim, $drop_ele_mode, $drop_mode, $elem_frac, $burst_size, $num_anomaly, $sigma_mag, $sigma_noise, $thresh, \@opt_types);
...
sub plot_prediction {
    ...
    # `$cmd`;
    # my $cmd = "sed 's/FILE_NAME/dect.$output_file/g; s/FIG_NAME/dect.$output_file/g; s/X_RANGE_S/0/g; s/X_RANGE_E//g; s/Y_RANGE_E/1/g; s/X_LABEL/Prediction Length/g; s/Y_LABEL/F1-Score/g; ' plot.pred.mother.plot > tmp.plot.dect.plot";
    # `$cmd`;
