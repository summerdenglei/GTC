...
sub get_y_max {
    ...
    if($drop_mode eq "ElemRandLoss") {
        ...
        elsif($file_name eq "tm_totem.")                                 { $y_max = 2.4; }
        elsif($file_name eq "tm_3g.cell.bs.bs3.all.bin10.txt")           { $y_max = 1.8; }
        elsif($file_name eq "tm_sjtu_wifi.ap_load.all.bin600.top50.txt") { $y_max = 5.0; }
        elsif($file_name eq "Mob-Recv1run1.dat0_matrix.mat_dB.txt")      { $y_max = 0.16; }
        elsif($file_name eq "tm_telos_rssi.txt")                         { $y_max = 0.7; }
        elsif($file_name eq "tm_multi_loc_rssi.txt")                     { $y_max = 0.16; }
        elsif($file_name eq "tm_ron1.latency.")                          { $y_max = 1.0; }
        elsif($file_name eq "tm_ucsb_meshnet.connected.txt")             { $y_max = 4.0; }
        elsif($file_name eq "tm_umich_rss.txt")                          { $y_max = 0.11; }
