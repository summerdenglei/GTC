...
# @files = ("tm_abilene.od.", "tm_totem.", "tm_ron1.latency.", "tm_ucsb_meshnet.");
                                ...
                                for my $sigma_mag (@sigma_mags) {
                                    for my $sigma_noise (@sigma_noises) {
                                        for my $thresh (@threshs) {
                                            ...
                                            plot_row_rand($func, $file_name, $num_frames, $width, $height, $group_size, $rank, $period, $opt_swap_mat, $opt_dim, $drop_ele_mode, $drop_mode, $loss_rate, $burst_size, $num_anomaly, $sigma_mag, $sigma_noise, $thresh, \@opt_types);
...
sub plot_row_rand {
    ...
    my $output_file = "RowRandLoss.$func.$file_name.$num_frames.$width.$height.$group_size.r$rank.period$period.$opt_swap_mat.$opt_dim.$drop_ele_mode.$drop_mode.loss$loss_rate.burst$burst_size.na$num_anomaly.anom$sigma_mag.noise$sigma_noise.thresh$thresh";
    ...
    my $cmd = "sed 's/FILE_NAME/pred.$output_file/g; s/FIG_NAME/pred.$output_file/g; s/X_RANGE_S/0/g; s/X_RANGE_E//g; s/Y_RANGE_E/$y_max/g; s/X_LABEL/Data Loss Probability/g; s/Y_LABEL/MAE/g; ' plot.pred.mother.plot > tmp.plot.pred.plot";
    `$cmd`;
...
sub get_y_max {
    ...
    if($drop_mode eq "RowRandLoss") {
        if   ($file_name eq "tm_abilene.od.")                            { $y_max = 1.5; }
        elsif($file_name eq "tm_totem.")                                 { $y_max = 2.2; }
        elsif($file_name eq "tm_ron1.latency.")                          { $y_max = 1.4; }
        elsif($file_name eq "tm_ucsb_meshnet.")                          { $y_max = 1.1; }
    }
