...
# @files = ("tm_abilene.od.", "tm_totem.", "tm_3g.cell.bs.bs3.all.bin10.txt", "tm_sjtu_wifi.ap_load.all.bin600.top50.txt", "Mob-Recv1run1.dat0_matrix.mat_dB.txt", "Mob-Recv1run1.dat0_matrix.mat_dB.txt", "tm_telos_rssi.txt", "tm_multi_loc_rssi.txt");
...
sub plot_anomaly_size {
    ...
    # my @sigma_mags = (0, 1, 2, 3, 4, 5, 6, 8, 10);
    my @sigma_mags = (4);
...
sub get_y_max {
    ...
    if($type eq "AnomalySize") {
        if($drop_mode eq "ind") {
            ...
            elsif($file_name eq "tm_3g.cell.bs.bs3.all.bin10.txt")           { $y_max = 1.1; }
