...
function [mse, mae, cc, ratio, tp, tn, fp, fn, precision, recall, f1score, jaccard, gamma, normalized_y, y_val, best_thresh] = srmf_based_pred(input_TM_dir, filename, num_frames, width, height, group_size, r, period, option_swap_mat, option_type, option_dim, drop_ele_mode, drop_mode, elem_frac, loss_rate, burst_size, num_anomaly, sigma_magnitude, sigma_noise, num_itr, thresh, seed, param1, param2, param3)
    ...
    output_dir = '/u/yichao/anomaly_compression/condor_data/subtask_compressive_sensing/condor_loop/pr/';
    space = 0;
    normalized_y = 0;
    y_val = 0;
    ...
    for gop = 1:num_groups
        ...
        elseif strcmpi(option_type, 'lens3')
            ...
            Q = toeplitz(CC,RR);
            K = P*zeros(n,m)*Q';
            [x,y,z,w,enable_B,sig,gamma] = lens3(D,this_r,A,B,C,E,P,Q,K,num_itr,[],soft,rho);
            if (enable_B)
                est1 = x+y;
                est2 = A*x+B*y;
            else
        ...
        elseif strcmpi(option_type, 'lens3_knn')
            ...
            Q = toeplitz(CC,RR);
            K = P*zeros(n,m)*Q';
            [x,y,z,w,enable_B,sig,gamma] = lens3(D,this_r,A,B,C,E,P,Q,K,num_itr,[],soft);
