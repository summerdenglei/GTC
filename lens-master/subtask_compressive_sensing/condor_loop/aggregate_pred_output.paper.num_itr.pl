...
# Variables
#############
my $input_dir  = "/u/yichao/anomaly_compression/condor_data/subtask_compressive_sensing/condor_loop/output";
my $output_dir = "/u/yichao/anomaly_compression/condor_data/subtask_compressive_sensing/output_loop";
my $figure_dir = "/u/yichao/anomaly_compression/condor_data/subtask_compressive_sensing/figures_loop";
...
for my $file_name (@files) {    
    ...
    ($num_frames, $width, $height, $group_size, $rank, $period) = get_trace_param($file_name, "unknown");
                                ...
                                for my $sigma_mag (@sigma_mags) {
                                    for my $sigma_noise (@sigma_noises) {
                                        for my $thresh (@threshs) {
                                            ...
                                            plot_num_itr($func, $file_name, $num_frames, $width, $height, $group_size, $rank, $period, $opt_swap_mat, $opt_dim, $drop_ele_mode, $drop_mode, $elem_frac, $loss_rate, $burst_size, $num_anomaly, $sigma_mag, $sigma_noise, $thresh, \@opt_types);
...
sub plot_num_itr {
    my ($func, $file_name, $num_frames, $width, $height, $group_size, $rank, $period, $opt_swap_mat, $opt_dim, $drop_ele_mode, $drop_mode, $elem_frac, $loss_rate, $burst_size, $num_anomaly, $sigma_mag, $sigma_noise, $thresh, $opt_types_ref) = @_;
    ...
    # my @sigma_mags = (0, 0.1, 0.5, 1, 1.5, 2, 2.5, 3, 5);
    # my @num_itrs = (1, 10, 100, 1000, 10000, 100000, 1000000);
    # my @num_itrs = (1, 10, 20, 40, 80, 160, 320, 640, 1280);
    my @num_itrs = (1, 10, 20, 50, 100, 150, 200, 250, 300, 350, 400, 450, 500, 550, 600);

    
    my $output_file = "NumItr.$func.$file_name.$num_frames.$width.$height.$group_size.r$rank.period$period.$opt_swap_mat.$opt_dim.$drop_ele_mode.$drop_mode.elem$elem_frac.lr$loss_rate.burst$burst_size.na$num_anomaly.anom$sigma_mag.noise$sigma_noise.thresh$thresh";
    open FH1, ">$output_dir/pred.$output_file.txt" or die $!;
    # open FH2, ">$output_dir/dect.$output_file.txt" or die $!;
    

    foreach my $nii (0 .. @num_itrs-1) {
        my $num_itr = $num_itrs[$nii];
        ...
        foreach my $ti (0 .. @opt_types-1) {
            ...
            my %rets = get_results($func, $file_name, $num_frames, $width, $height, $group_size, $rank, $period, $opt_swap_mat, $opt_type, $opt_dim, $drop_ele_mode, $drop_mode, $elem_frac, $loss_rate, $burst_size, $num_anomaly, $sigma_mag, $sigma_noise, $sigma_mag, $num_itr, $thresh);
    ...
    my $y_max = get_y_max("NumItr", $drop_mode, $file_name);
    my $cmd = "sed 's/FILE_NAME/pred.$output_file/g; s/FIG_NAME/pred.$output_file/g; s/X_RANGE_S//g; s/X_RANGE_E//g; s/Y_RANGE_E/$y_max/g; s/X_LABEL/Number of Iterations/g; s/Y_LABEL/NMAE/g;' $plot_file > tmp.plot.pred.plot";
    `$cmd`;
    ...
    foreach my $oi (0 .. @opt_types-1) {
        ...
        print FH "\"\" " if($oi > 0);
        print FH "using 1:".($col+$oi)." with linespoints ls ".($oi%$NUM_CURVE+1)." title '{/Helvetica=28 NMAE}' axes x1y1";
    ...
    #     print FH "\"\" ";
    #     # print FH "\"\" " if($oi > 0);
    #     print FH "using 1:".($col+$oi)." with linespoints ls 13 title '{/Helvetica=28 gamma}'  axes x1y2";
...
sub get_y_max {
    ...
    if($type eq "NumItr") {
        if($drop_mode eq "ind") {
            if   ($file_name eq "tm_abilene.od.")                            { $y_max = 1; }
            elsif($file_name eq "tm_totem.")                                 { $y_max = 1; }
            elsif($file_name eq "tm_3g.cell.bs.bs3.all.bin10.txt")           { $y_max = 1; }
            elsif($file_name eq "tm_sjtu_wifi.ap_load.all.bin600.top50.txt") { $y_max = 1; }
            elsif($file_name eq "Mob-Recv1run1.dat0_matrix.mat_dB.txt")      { $y_max = 1; }
            elsif($file_name eq "tm_telos_rssi.txt")                         { $y_max = 1; }
            elsif($file_name eq "tm_multi_loc_rssi.txt")                     { $y_max = 1; }
            elsif($file_name eq "tm_ron1.latency.")                          { $y_max = 1; }
            elsif($file_name eq "tm_ucsb_meshnet.connected.txt")             { $y_max = 1; }
            elsif($file_name eq "tm_umich_rss.txt")                          { $y_max = 1; }
            elsif($file_name eq "static_trace13.ant1.mag.txt")               { $y_max = 1; }
        }
        else {
...
sub get_results {
    ...
    my ($func, $file_name, $num_frames, $width, $height, $group_size, $rank, $period, $opt_swap_mat, $opt_type, $opt_dim, $drop_ele_mode, $drop_mode, $elem_frac, $loss_rate, $burst_size, $num_anomaly, $sigma_mag, $sigma_noise, $sigma_mag, $num_itr, $thresh) = @_;
    ...
    for my $seed (@seeds) {
        my $this_file_name = "$input_dir/$func.$file_name.$num_frames.$width.$height.$group_size.r$rank.period$period.$opt_swap_mat.$opt_type.$opt_dim.$drop_ele_mode.$drop_mode.elem$elem_frac.loss$loss_rate.burst$burst_size.na$num_anomaly.anom$sigma_mag.noise$sigma_noise.itr$num_itr.thresh$thresh.seed$seed.txt";
