...
function [mse, mae, cc, ratio, tp, tn, fp, fn, precision, recall, f1score, jaccard, normalized_y, y_val, best_thresh, gamma] = srmf_based_pred(input_TM_dir, filename, num_frames, width, height, group_size, r, period, option_swap_mat, option_type, option_dim, drop_ele_mode, drop_mode, elem_frac, loss_rate, burst_size, num_anomaly, sigma_magnitude, sigma_noise, sel_gamma, thresh, seed, param1, param2, param3)
    ...
    for gop = 1:num_groups
        ...
        elseif strcmpi(option_type, 'lens3')
            ...
            Q = toeplitz(CC,RR);
            K = P*zeros(n,m)*Q';
            [x,y,z,w,enable_B,sig,gamma] = lens3(D,this_r,A,B,C,E,P,Q,K,sel_gamma, [],soft,rho);
            if (enable_B)
                est1 = x+y;
                est2 = A*x+B*y;
            else
