function [TT,V1,V2,V3,S,dist,truerel] = TenAlS(T,TE, E, r, ninit, nitr, tol)
...
if (isempty(ninit)==1) ninit=10; end;
if (isempty(nitr)==1) nitr=100;  end;
if (isempty(tol)==1)   tol = 1e-8; end;
...
for itr1=1:nitr
    ...
    if (sqrt(normERR/normTE)<tol)
        break;
    end
    
    %          TT = zeros(n1,n2,n3);
    %    for i=1:n3
    %         TT(:,:,i) = V1*diag(V3(i,:).*S(:)')*V2';
    %    end
    TT = CPcomp(S,V1,V2,V3);
    dist = sqrt(normERR/normTE);
    truerel(itr1)=norm(TT(:)-T(:))/norm(T(:));
