function [X,Y,TC,TotalRes,TrueRel,truerel] = TCTF_tensorJFT_solver(Oridata,B,data,known,Nway,coreNway,opts)
%% Data preprocessing and initialization频域变换使用JFT
% load('spatial_matrix_half.mat')
% load('temporal_matrix_half.mat')
% load('8road_spatial_matrix.mat')
% load('spatial_144_175_9.mat')
load('L_FZ.mat')%laplace矩阵
...
TC=rand(Nway);
TC(known)=data;
TC=tensor_JFT(TC,S);%频域C 第一slice为实数，其他为复数 为什么？
...
%         Cons{2}=TM;
%         EE_FFT
%          输入参数X{n},Y{n}是初始化的L和R矩阵，C(:,:,n)=X{n}*Y{n}，
