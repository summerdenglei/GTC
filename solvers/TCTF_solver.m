...
%          res=norm(TC(known)-data);
    ...
    if rank_adj(n) == -1 && rho<1 && rank_str==-1
        ...
        for i=1:n3
            ...
            sigmas((i-1)*max_k+1:(i-1)*max_k+length(s))=s;%用X{n}'*X{n}奇异值对sigma循环赋值1-23，24-46，。。。
        end
        [dR,id]=sort(sigmas,'descend');%奇异值降序排列
