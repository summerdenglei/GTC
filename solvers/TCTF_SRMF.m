function [U,V] = TCTF_SRMF(U,V,X,r,M,Cons,alpha,lambda)
%               TCTF_SRMF(X{n},Y{n}',C(:,:,n),coreNway(n),ones(n1,n2),Cons,alpha,lamda);
...
%
%   PtP = P'*P and y=Ptp = P'*p
%
cPtP = zeros(r*r*nv,1);
Ptp  = zeros(r*nv,1);
for k = 1:nv
    Mnz     = find(M(:,k));%M中第k列的采样位置（值为1）索引
    p_k     = X(Mnz,k);%对应位置原始数据
    P_k     = UUv(Mnz,:);%选取对应UUv的Mnz行
    
    I       = ((k-1)*r*r+1):(k*r*r);%当r=5，1-25,26-50.。。
    cPtP(I) = reshape(P_k'*P_k,r*r,1);%PtP = P'*P 
    
    I       = ((k-1)*r+1):(k*r);%当r=5,1-5,6-10
    Ptp(I)  = P_k'*p_k;%y=Ptp = P'*p
end
...
PtP = sparse(I+K,J+K,cPtP,nv*r,nv*r);%将cPtP填充为n2个r*r块对角阵
